...
describe('terminal reporter (§13/§15.3)', () => {
  it('matches the §13 golden output for the breaking case', () => {
    const r = report([
      change({
        ...
        severity: 'error',
        message: 'Request property `email` changed from optional to required',
        path: '/users',
        method: 'POST',
      ...
      change({
        ...
        severity: 'error',
        message: 'Response property `phoneNumber` was removed',
        path: '/users/{id}',
        method: 'GET',
  ...
  it('clean case prints the informational count (§13)', () => {
    const r = report([
      change({
        ...
        severity: 'info',
        message: 'API description changed',
        path: '/',
      }),
    ]);
    const out = renderTerminal(r, {
  ...
  it('--include-non-breaking renders warnings and info rows', () => {
    ...
    const without = renderTerminal(r, {
      includeNonBreaking: false,
      showIgnored: false,
      color: false,
      quiet: false,
    });
    const withN = renderTerminal(r, {
  ...
  it('suggestLine formats bumps and suppresses none', () => {
    ...
    const r = report([]);
    expect(suggestLine({ ...r, semver: { bump: 'major', reason: '', triggers: [] } })).toBe(
      'Suggested version bump: MAJOR',
    );
    expect(suggestLine({ ...r, semver: { bump: 'minor', reason: '', triggers: [] } })).toBe(
  ...
  it('has summary table, details groups, and footer', () => {
    const r = report([
      change({
        ruleId: 'removed-path',
        severity: 'error',
        message: 'Path removed: /x | with pipe',
        path: '/x',
      }),
...
describe('reporter branches: colors, ignored, webhook/component labels', () => {
  ...
  it('terminal: color markers, webhook/component group labels, ignored section', async () => {
    ...
    const colored = renderTerminal(base() as never, {
      includeNonBreaking: false,
      showIgnored: true,
      color: true,
      quiet: false,
    ...
    (r2 as { changes: Array<{ location: { method?: string } }> }).changes[0]!.location.method =
      undefined;
    const grouped = renderTerminal(r2, {
      includeNonBreaking: false,
      showIgnored: false,
      color: false,
      quiet: false,
