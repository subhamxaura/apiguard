...
__export(markdown_exports, {
  markdownBumpLine: () => markdownBumpLine,
...
var severityValueSchema = import_zod.z.enum(["error", "warning", "info", "off"]);
var rulesSchema = import_zod.z.record(import_zod.z.string().min(1), severityValueSchema);
var ignoreSchema = import_zod.z.strictObject({
  paths: import_zod.z.array(import_zod.z.string()).optional().default([]),
  operations: import_zod.z.array(import_zod.z.string()).optional().default([]),
  schemas: import_zod.z.array(import_zod.z.string()).optional().default([]),
  changes: import_zod.z.array(import_zod.z.string()).optional().default([]),
  "show-ignored": import_zod.z.boolean().optional().default(false)
});
var outputSchema = import_zod.z.strictObject({
  format: import_zod.z.enum(["terminal", "json", "markdown"]).optional().default("terminal"),
  "include-non-breaking": import_zod.z.boolean().optional().default(false),
  color: import_zod.z.enum(["auto", "always", "never"]).optional().default("auto")
});
var versioningSchema = import_zod.z.strictObject({
  suggest: import_zod.z.boolean().optional().default(true)
});
var loaderSchema = import_zod.z.strictObject({
  "allow-remote-refs": import_zod.z.boolean().optional().default(false)
});
var githubSchema = import_zod.z.strictObject({
  comment: import_zod.z.boolean().optional().default(true),
  "check-run": import_zod.z.boolean().optional().default(true),
  "update-existing-comment": import_zod.z.boolean().optional().default(true)
});
var configSchema = import_zod.z.strictObject({
  ...
  "fail-on": import_zod.z.enum(["error", "warning", "never"]).optional(),
...
function readConfigFile(filePath) {
  let text;
  try {
    text = import_node_fs.default.readFileSync(filePath, "utf8");
  } catch {
...
function discoverAndLoadConfig(opts = {}) {
  ...
  if (opts.explicit || opts.env) {
    ...
    const abs = import_node_path.default.isAbsolute(target) ? target : import_node_path.default.resolve(cwd, target);
    if (!import_node_fs.default.existsSync(abs)) {
      throw new ConfigError(`config file not found: ${abs}`);
    }
    return loadConfigFrom(abs);
  }
  for (const candidate of CONFIG_FILE_CANDIDATES) {
    const abs = import_node_path.default.resolve(cwd, candidate);
    if (import_node_fs.default.existsSync(abs)) {
      return loadConfigFrom(abs);
    }
  }
...
function sha256OfValue(value) {
  return (0, import_node_crypto.createHash)("sha256").update(canonicalize(value)).digest("hex");
}
...
function parseSpecText(text, source) {
  ...
  if (isJsonByExt) {
    ...
    try {
      parsed = (0, import_yaml2.parse)(text, { schema: "core" });
    } catch (e) {
    ...
    for (const [k, v] of Object.entries(obj)) {
      if (k === "$ref" && typeof v === "string") {
        ...
        if (filePart && !/^[a-z][a-z0-9+.-]*:/i.test(filePart)) {
          const resolved = import_node_path2.default.resolve(specDir, filePart);
          const rel = import_node_path2.default.relative(specDir, resolved);
          if (rel.startsWith("..") || import_node_path2.default.isAbsolute(rel)) found.push(v);
        }
      } else if (v !== null && typeof v === "object") {
        stack.push(v);
      }
...
async function loadSpec(filePath, options = {}) {
  const abs = import_node_path2.default.resolve(filePath);
  let text;
  try {
  ...
  const specDir = import_node_path2.default.dirname(abs);
  ...
  let document;
  try {
    document = await import_swagger_parser.default.bundle(abs);
  } catch (e) {
...
function compareChanges(a, b) {
  ...
  if (sev !== 0) return sev;
  const path4 = (a.location.path ?? "").localeCompare(b.location.path ?? "", "en", {
    sensitivity: "variant"
  });
  if (path4 !== 0) return path4;
...
function diffOperationPair(ctx, path4, method, oldItem, newItem) {
  ...
  const baseLoc = { path: path4, method };
  if (!deepEqual(oldOp.operationId, newOp.operationId)) {
    emit(ctx, {
      ...
      location: {
        ...
        pointerOld: pointerJoin("paths", path4, method.toLowerCase(), "operationId"),
        pointerNew: pointerJoin("paths", path4, method.toLowerCase(), "operationId"),
        context: "operation"
      },
  ...
  if (newDep && !oldDep) {
    emit(ctx, {
      ...
      location: {
        ...baseLoc,
        pointerOld: pointerJoin("paths", path4, method.toLowerCase(), "deprecated"),
        pointerNew: pointerJoin("paths", path4, method.toLowerCase(), "deprecated"),
        context: "operation"
      }
    });
  ...
  if (!deepEqual(oldOp.tags, newOp.tags)) {
    emit(ctx, {
      ...
      location: {
        ...baseLoc,
        pointerOld: pointerJoin("paths", path4, method.toLowerCase(), "tags"),
        pointerNew: pointerJoin("paths", path4, method.toLowerCase(), "tags"),
        context: "operation"
      },
  ...
  if (!deepEqual(oldOp.summary, newOp.summary)) {
    emit(ctx, {
      ...
      location: {
        ...baseLoc,
        pointerOld: pointerJoin("paths", path4, method.toLowerCase(), "summary"),
        pointerNew: pointerJoin("paths", path4, method.toLowerCase(), "summary"),
        context: "operation"
      },
  ...
  if (!deepEqual(oldOp.description, newOp.description)) {
    emit(ctx, {
      ...
      location: {
        ...baseLoc,
        pointerOld: pointerJoin("paths", path4, method.toLowerCase(), "description"),
        pointerNew: pointerJoin("paths", path4, method.toLowerCase(), "description"),
        context: "operation"
      },
...
function compareSecurity(ctx, path4, method, oldReqs, newReqs) {
  ...
  if (!oldEmpty && newEmpty) {
    emit(ctx, {
      ...
      location: {
        ...
        path: path4,
        method,
        pointerOld: pointerJoin("paths", path4, method.toLowerCase(), "security"),
        pointerNew: pointerJoin("paths", path4, method.toLowerCase(), "security")
      }
    });
    return;
  }
...
function isIgnored(change, config) {
  ...
  if (ignore.changes.length > 0) {
    if (import_micromatch.default.isMatch(change.ruleId, ignore.changes, { nocase: false })) return true;
  }
  const path4 = changePath(change);
  if (ignore.schemas.length > 0) {
    const name = change.location.componentName;
    if (name && import_micromatch.default.isMatch(name, ignore.schemas)) return true;
    ...
    if (pointer && (pointer.includes(`/${SCHEMAS_SEG}`) || pointer.startsWith(SCHEMAS_SEG))) {
      ...
      if (rootName) {
        if (import_micromatch.default.isMatch(rootName, ignore.schemas)) return true;
        const lastSegments = ignore.schemas.map((s) => s.split("/").pop() ?? s);
        if (import_micromatch.default.isMatch(rootName, lastSegments)) return true;
        const prefixGlobs = ignore.schemas.map((s) => `components/schemas/${s}**`);
        if (import_micromatch.default.isMatch(pointer, prefixGlobs)) return true;
      }
    }
  }
  if (ignore.operations.length > 0 && change.location.method !== void 0) {
    const opKey = `${change.location.method.toUpperCase()} ${path4}`;
    if (ignore.operations.includes(opKey)) return true;
  }
  if (ignore.paths.length > 0 && path4 && path4 !== "") {
    if (import_micromatch.default.isMatch(path4, ignore.paths)) return true;
  }
  return false;
}
...
async function runDiff(oldSpec, newSpec, opts, io) {
  ...
  io.stdout(body);
  if (opts.output) {
    import_node_fs3.default.writeFileSync(opts.output, body + "\n", "utf8");
...
function filesFor(_dir, withExamples) {
  ...
  if (withExamples) {
    files.push({ relPath: import_node_path3.default.join("examples", "baseline.yaml"), content: EXAMPLE_BASELINE });
    files.push({ relPath: import_node_path3.default.join("examples", "current.yaml"), content: EXAMPLE_CURRENT });
  }
  return files;
}
async function runInit(dir, opts, io) {
  const target = import_node_path3.default.resolve(dir);
  const planned = filesFor(target, opts.examples === true);
  const conflicts = planned.filter((f) => import_node_fs4.default.existsSync(import_node_path3.default.join(target, f.relPath)));
...
use --force to overwrite`
  ...
  for (const f of planned) {
    const abs = import_node_path3.default.join(target, f.relPath);
    import_node_fs4.default.mkdirSync(import_node_path3.default.dirname(abs), { recursive: true });
    import_node_fs4.default.writeFileSync(abs, f.content, "utf8");
